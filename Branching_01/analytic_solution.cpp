...
xt::xarray<double> fix_t_varies_x(double t, double a, double v, double lower_bound, double upper_bound, double delta_x) {
    ...
    xt::xarray<double> arr2 = a * 6. / ( (arr + factor_1) * (arr + factor_1) );
    return arr2;
}
...
xt::xarray<double> fix_x_varies_t(double x, double a, double v, double lower_bound, double upper_bound, double delta_t_) {
    ...
    xt::xarray<double> arr2 = a * 6. / ( (x + v * arr) * (x + v * arr) );
    return arr2;
} 
...
int main()
{
    ...
    for (int i=0; i<=10; i++) {
        ...
        if (i==0) {
            arr = fix_t_varies_x(t, 1., sqrt(2), 1., 10., 0.01);
            arr = arr.reshape({1, static_cast<int> ((10 - 1)/0.01) + 1});
        }
        else {
            arr = fix_t_varies_x(t, 1., sqrt(2), 0., 10., 0.01);
    ...
    for (int i=0; i<=10; i++) {
        ...
        xt::xarray<double> arr;
        if (i==0) {
            arr = fix_x_varies_t(x, 1., sqrt(2), 1., 10., 0.01);
            arr = arr.reshape({1, static_cast<int> ((10 - 1)/0.01) + 1});
        }
        else {
