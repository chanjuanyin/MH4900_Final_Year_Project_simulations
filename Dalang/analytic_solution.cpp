...
xt::xarray<double> fix_t_varies_x(double t, double a, double v, double lower_bound, double upper_bound, double delta_x) {
    ...
    xt::xarray<double> arr2 = (xt::exp(a * t + v * arr) + xt::exp(a * t - v * arr)) * (factor_1 * xt::exp(factor_2 * t) + factor_2 * xt::exp(-factor_1 * t));
    return arr2;
}
...
xt::xarray<double> fix_x_varies_t(double x, double a, double v, double lower_bound, double upper_bound, double delta_t_) {
    ...
    double double_2 = -a + sqrt(a*a + v*v);
    xt::xarray<double> factor_2 = {double_2};
    xt::xarray<double> arr2 = (xt::exp(a * arr + v * x) + xt::exp(a * arr - v * x)) * (factor_1 * xt::exp(factor_2 * arr) + factor_2 * xt::exp(-factor_1 * arr));
    return arr2;
} 
...
int main()
{
    ...
    for (int i=0; i<=10; i++) {
        ...
        xt::xarray<double> arr = fix_t_varies_x(t, 0.3, 0.3, -5, 5, 0.01);
        ...
        // cout << "}";
        // cout << arr << endl;
        string file_name = "../Dalang/output/t_equals_";
    ...
    for (int i=-5; i<=5; i++) {
        ...
        xt::xarray<double> arr = fix_x_varies_t(x, 0.3, 0.3, 0, 10, 0.01);
        arr = arr.reshape({1, static_cast<int> ((5 + 5)/0.01) + 1});
        // cout << "arr = { ";
        // for (double d : arr) {
        //   cout << d << ", ";
        ...
        // cout << "}";
        // cout << arr << endl;
        string file_name = "../Dalang/output/x_equals_";
        if (i<0) {
            file_name += "neg_";
