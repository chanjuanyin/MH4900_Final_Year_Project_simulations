...
xt::xarray<double> fix_x_varies_t(double x, double y, double lower_bound, double upper_bound, double delta_t_) {
    xt::xarray<double> arr = xt::arange(lower_bound, upper_bound + delta_t_, delta_t_);
    xt::xarray<double> arr2 = 6 / (pow(x + y + sqrt(3) * arr, 2));
    return arr2;
} 

int main()
{
    string directoryPath = "../Branching_08/output";
    ...
    arr = fix_x_varies_t(x, y, 0., 10., 0.01);
    arr = arr.reshape({1, static_cast<int> ((10 - 0)/0.01) + 1});
    string file_name = "../Branching_08/output/analytic.csv";
