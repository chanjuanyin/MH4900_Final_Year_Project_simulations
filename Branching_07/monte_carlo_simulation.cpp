...
double simulate_recursion(double t, double x, double lambda) {
    ...
    if (tau<t) {
        ...
        if (J < 0) {
            ...
            return (1./lambda) * exp(lambda * tau) * tau * 3 * v1 * v2;
        }
        else {
            double x_tilda = tau * uniform_distribution(generator);
            double v1 = simulate_recursion(t-tau, x-x_tilda, lambda);
            ...
            return (1./lambda) * exp(lambda * tau) * tau * 4 * v1 * v2 * v3;
        }
        ...
        if (eta < 0) {
            return exp(lambda * t) * 2 * 2 * ( pow(pow(x+t, 2)-4,-1) + pow(pow(x-t, 2)-4,-1) );
        }
        else {
            return exp(lambda * t) * 2 * 2 * sqrt(2) * ( pow(pow(x+t, 2)-4,-1) - pow(pow(x-t, 2)-4,-1) );
        }
    }
}
...
int main()
{
    string directoryPath = "../Branching_07/output";
    ...
    for (int k = 0; k < num_estimations; k++) {
        ...
        string file_name = "../Branching_07/output/monte_carlo.csv";
